...
import { BasicProductList, CustomButton, CustomInput } from '../../components';
import { COLORS, EMPTY_STRING, NETWORK } from '../../constants';
...
const CheckoutScreen = (props) => {
  ...
  const setOriginalAddress = () => {
    if (_isEmpty(userInfo)) {
      setStreetAddress(EMPTY_STRING);
    }
    const { block = 'A', flat_number = '1', floor_number = '202' } = userInfo;
    setStreetAddress(`Block: ${block}, Flat Number:${flat_number}, Floor Number: ${floor_number}`);
      ...
      <ScrollView style={styles.bodyContainer} nestedScrollEnabled={true}>
        ...
        <View style={styles.listContainer}>
          <View style={styles.list}>
            ...
            <Text style={styles.secondaryTextSm}>
              {_get(userInfo, 'email_address', EMPTY_STRING)}
            </Text>
          </View>
          <View style={styles.list}>
            <Text style={styles.secondaryTextSm}>Phone</Text>
            <Text style={styles.secondaryTextSm}>   {_get(userInfo, 'contact_number', EMPTY_STRING)}</Text>
          </View>
        </View>
