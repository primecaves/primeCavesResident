...
export default class PaymentHistory extends React.Component {
  ...
  fetchPaymentHistory = () => {
    const { userInfo, route = EMPTY_OBJECT } = this.props;
    this.setState({ isLoading: true });
    const request = _get(route, 'params', {});
        ...
        <Block flex style={{ width: '90%' }}>
          <ScrollView showsVerticalScrollIndicator={false}>
            {_map(serviceData, item => (
              <Notification
                body={`${_get(item, 'title', 'Charges')} paid for ${_get(
                  item,
                  'period',
                  EMPTY_STRING,
                )} through ${_get(item, 'payment_mode', EMPTY_STRING)} mode.`}
                iconName="ship"
