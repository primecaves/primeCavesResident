...
import { EMPTY_OBJECT, EMPTY_STRING, argonTheme } from '../../constants';
...
import { getSearchDetails } from './search.services';
import { DynamicKeyCard } from '../../components';
...
export default class Search extends React.Component {
  ...
  handleSearchChange = search => {
    this.setState({ search }, this.handleFetchResult);
    this.animate();
  };
    ...
    return (
      <Block style={styles.notfound}>
        <Text
          ...
          bold
        >
          {!_isEmpty(search) && results.length === 0
            ? 'Not Found Anything Like That...Try Something Else'
            : 'Search...'}
        </Text>
      </Block>
    );
  ...
  renderDeals = () => {
    const { results, isLoading, displayNameKey, keyToRemove } = this.state;
    return (
  ...
  renderResults = () => {
    ...
    if (results.length === 0 || _isEmpty(search)) {
      return (
        <Block style={{ width: width - 40 }}>{this.renderNotFound()}</Block>
      );
    }
